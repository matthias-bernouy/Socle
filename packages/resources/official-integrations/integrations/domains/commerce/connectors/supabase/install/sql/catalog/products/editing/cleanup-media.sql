...
create or replace function commerce.claim_product_media_cleanup(p_session_id uuid, p_owner_id text, p_media_ids jsonb default null)
...
begin
    if p_session_id is null then
        ...
        for v_session in
            select session.id from commerce.product_upload_sessions session
            where session.owner_id = p_owner_id and exists (
                select 1 from commerce.product_media_uploads pending where pending.session_id = session.id
        ...
        loop
            v_items := v_items || (commerce.claim_product_media_cleanup(v_session, p_owner_id)->'items');
        end loop;
        return jsonb_build_object('items', v_items);
    end if;
    perform commerce.lock_product_upload_session(p_session_id, p_owner_id, true);
    ...
    if p_media_ids is not null then
        ...
        if exists (select 1 from unnest(v_ids) candidate(media_id) where not exists (
            select 1 from commerce.product_media_uploads where session_id = p_session_id and media_id = candidate.media_id
        )) then raise exception 'conflict: only pending product images can be discarded'; end if;
    end if;
    update commerce.product_media_uploads set state = 'deleting'
    where session_id = p_session_id and (
        ...
        (v_ids is null and (expires_at <= now() or state = 'deleting' or exists (
            select 1 from commerce.product_upload_sessions where id = p_session_id and expires_at <= now()
        )))
    );
    select coalesce(jsonb_agg(jsonb_build_object(
        'sessionId', p_session_id, 'mediaId', media.id, 'storageBucket', media.storage_bucket, 'storagePath', media.storage_path
    )), '[]'::jsonb) into v_result
    from commerce.product_media_uploads pending join commerce.media media on media.id = pending.media_id
    where pending.session_id = p_session_id and pending.state = 'deleting';
    return jsonb_build_object('items', v_result);
end;
$$;

create or replace function commerce.finish_product_media_cleanup(p_session_id uuid, p_owner_id text, p_media_id bigint)
returns void language plpgsql set search_path = '' as $$
begin
    perform commerce.lock_product_upload_session(p_session_id, p_owner_id, true);
    delete from commerce.product_media_uploads
