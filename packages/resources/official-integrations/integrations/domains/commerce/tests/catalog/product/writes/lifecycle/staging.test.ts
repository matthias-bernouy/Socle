...
function useStaging(options: { uploadFails?: boolean; cleanupFails?: boolean; signingFails?: boolean } = {}): void {
    setRestResponder(async (request) => {
        ...
        if (name === "complete_media_upload" || name === "finish_media_cleanup") {
...
describe.each(["product", "offer"])("%s staged image transport", (kind) => {
    test("cleans up an uploaded image if preview signing fails before returning its session", async () => {
        useStaging({ signingFails: true });
        const response = await requestCommerce(`/admin/${kind}/image/stage?sessionId=${session}`, {
            userId: "admin-a",
            userRole: "admin",
        ...
        expect(
            capturedFetches()
                .filter((call) => call.url.includes("/rpc/"))
                .every((call) => call.body.p_resource_kind === kind),
        ).toBe(true);
    ...
    test("keeps cleanup retryable if Storage deletion fails", async () => {
        ...
        const response = await requestCommerce(`/admin/${kind}/images/discard`, {
            userId: "admin-a",
            userRole: "admin",
            body: { sessionId: session, mediaIds: [5] },
        });
        expect(response.status).toBe(502);
        expect(callKinds()).toEqual(["claim_media_cleanup", "storage:DELETE"]);
    });

    test("never deletes a saved original when the database refuses the cleanup claim", async () => {
        setRestResponder(() => jsonResponse({ message: "conflict: only pending images can be discarded" }, 400));
        const response = await requestCommerce(`/admin/${kind}/images/discard`, {
            userId: "admin-a",
            userRole: "admin",
            body: { sessionId: session, mediaIds: [5] },
        ...
        expect(callKinds()).toEqual(["claim_media_cleanup"]);
    });
});
