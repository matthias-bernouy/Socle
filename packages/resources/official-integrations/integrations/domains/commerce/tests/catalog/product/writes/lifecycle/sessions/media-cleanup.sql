...
begin
    ...
    update commerce.media_uploads set expires_at = now() - interval '1 second' where media_id = v_media;
    if commerce.claim_media_cleanup('product', null, 'admin-b')->'items' <> '[]'::jsonb then
        raise exception 'test: owner cleanup included another administrator';
    end if;
    v_result := commerce.claim_media_cleanup('product', null, 'admin-a');
    ...
    if commerce.claim_media_cleanup('product', v_session, 'admin-a') <> v_result then
    ...
    perform commerce.finish_media_cleanup('product', v_session, 'admin-a', v_media);
    if exists (select 1 from commerce.media_uploads where media_id = v_media)
    ...
    if has_table_privilege('anon', 'commerce.media_uploads', 'select')
        or has_table_privilege('authenticated', 'commerce.media_uploads', 'insert')
        or has_function_privilege('authenticated', 'commerce.stage_media(text, uuid,text,boolean,jsonb)', 'execute') then
