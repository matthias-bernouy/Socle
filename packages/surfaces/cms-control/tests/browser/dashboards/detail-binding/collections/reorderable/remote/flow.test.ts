...
import { installRemoteChoices } from "./fixture";
...
test("choice lookup fields share requests across rows while isolating search, pagination and stale responses", async () => {
    const browser = await chromium.launch();
    let release = () => {};
    try {
        const page = await browser.newPage({ viewport: { width: 1440, height: 1000 } });
        page.setDefaultTimeout(5000);
        const fixture = await installRemoteChoices(page, bundle, styles);
        await page.goto(reorderablePage);
        const rows = page.locator('[data-field-control="rows"] .row[data-index]');
        const firstBrand = rows.first().locator('[data-item-field="brand"]');
        const secondBrand = rows.last().locator('[data-item-field="brand"]');
        const supplier = rows.first().locator('[data-item-field="supplier"]');
        ...
        await rows.first().locator('[data-item-field="label"] input').fill("Retain row draft");
        release = fixture.holdQuery("Old");
        ...
        expect(await rows.first().locator('[data-item-field="label"] input').inputValue()).toBe("Retain row draft");
        fixture.fail("Broken");
        ...
        expect(fixture.saved[0]?.rows).toEqual([
            { id: "first", label: "Retain row draft", brand: "brand-0", supplier: "supplier-0", position: 0 },
            { id: "second", label: "Second", brand: "brand-26", supplier: "supplier-1", position: 1 },
        ]);
