...
test("mobile table failures retain bottom drafts, navigation, selection and scroll through retry", async () => {
    ...
    try {
        ...
        await page.route(
            ...
            async (route) => {
                await pending;
                await route.fulfill({ status: 503, json: { error: "Table save unavailable" } });
            },
            { times: 1 },
        );
        ...
        await page.getByText(/Table save unavailable/).waitFor();
